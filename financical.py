...
class FinancialData():
	...
	def __init__(self):
		...
		self.llb_dict = {
			...
			'CF_INT_REC_G':'已收股息','CF_DIV_PAID':'已派股息','CF_INV':'投资活动产生现金流',
	...
	def insert_tables(self, name, table_name_list,table_date_list, url_list):
		...
		conn = pymssql.connect(
                       server=r'.\SQLEXPRESS', database='pachongdb', user='sa', password='123456')
		#使用cursor()方法获取操作游标
		cursor = conn.cursor()  
		#插入信息
		for i in range(len(table_name_list)):
			...
			for num in range(nums):
				...
				sys.stdout.flush()
				value_dict['gp_name'] = name
				value_dict['gp_code'] = code
				for key, value in table[num].items():
					if key in self.table_dict[url_list[i]]:
						value_dict[key] = value
				sql1 = """
				INSERT INTO %s (gp_name,gp_code,YEAREND_DATE) VALUES ('%s','%s','%s')""" % (url_list[i],value_dict['gp_name'],value_dict['gp_code'],value_dict['YEAREND_DATE'])
				try:
					cursor.execute(sql1)
					# 执行sql语句
					conn.commit()
				...
				for key, value in value_dict.items():
					if key not in ['gp_name','gp_code','YEAREND_DATE']:
						sql2 = """
						UPDATE %s SET %s='%s' WHERE gp_name='%s' AND YEAREND_DATE='%s'""" % (url_list[i],key,value,value_dict['gp_name'],value_dict['YEAREND_DATE'])
						try:
							cursor.execute(sql2)
							# 执行sql语句
							conn.commit()
